...
use janu_flow::{model::link::PortDescriptor, zf_empty_state};
use janu_flow::{Context, Data, Node, Sink, Source};
use janu_flow::{State, ZFResult};
...
impl Sink for ExampleGenericSink {
    async fn run(
        ...
        _state: &mut State,
        input: janu_flow::runtime::message::DataMessage,
    ) -> janu_flow::ZFResult<()> {
...
async fn main() {
    ...
    let mut zf_graph =
        janu_flow::runtime::dataflow::Dataflow::new(ctx.clone(), "simple-pipeline".into(), None);
